...
async def filter(torrents: list, name: str, year: int):
    ...
    for torrent in torrents:
        ...
        parsed = parse(title)
        print(parsed)
        if "CAM" in str(parsed):
            results.append((index, False))
            continue
        if "Telesync" in str(parsed):
            results.append((index, False))
            continue
        if title_match(name, parsed.parsed_title):
...
def format_title(data: dict, config: dict):
    ...
    bad_chars = [',', '!', "[", "]", "'"]
    for i in bad_chars:
        ishdr = ishdr.replace(i, '')
    

    ishdr = ' '.join(ishdr.splitlines())
    ishdr = ishdr.replace(' ', '  //  ')
    quality= str(data["quality"])
    year = str(data["year"])
    title = f"{title_parsed.parsed_title} {year}\n{ishdr}\n{quality}\n💾 {bytes_to_size(data['size'])} "
